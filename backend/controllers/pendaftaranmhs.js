...
exports.editmhs = (req, res, next) => {
  ...
  dbmysql.getConnection((err, connection) => {
    ...
    connection.query('UPDATE pendaftaranmhs SET nama = ' + req.body.nama + ', nim = ' + req.body.nim + ', ipk = ' + req.body.ipk + ', nokwitansi = ' + req.body.nokwitansi + ', foto_mhs = ?, Where ID = ' + req.body.id , imagePath,(err, result) => {
      if (err) throw err;
    });
  });
  // const post = new PendaftaranMhs({
...
exports.tampilmhsid = (req, res, next) => {
  dbmysql.getConnection((err, connection) => {
    ...
    dbmysql.query("SELECT * FROM pendaftaranmhs WHERE id_mhs='+req.params.id'")
